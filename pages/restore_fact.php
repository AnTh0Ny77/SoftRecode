...
if ($_SESSION['user']->user__facture_acces < 40) {
...
if (!empty($_POST['restore'])) {
        //si une restauration à été demandé : 
        $facture_list = $General->restore();

        foreach ($facture_list as $facture) {
                //si il s'agit d'une facture classique :

                // 3 enregistrer la facture au format pdf dans un folder 
                $temp = $Cmd->GetById($facture->devis__id);
                ...
                //ligne de devis : 
                $arrayOfDevisLigne = $Cmd->devisLigne_actif($facture->devis__id);
                        ...
                        <page_header>
                                <table class="page_header" style="width: 100%;">
                                        ...
                                        <tr>
                                                ...
                                                <td style="text-align: left; width:50%">
                                                <?php
                                                        if ($temp->cmd__modele_facture != 'AVR') 
                                                        {
                                                             echo '<h3>Facture : ' .$temp->cmd__id_facture . ' le ' . $formate; '</h3><br>';
                                                        }
                                                        else
                                                        {
                                                                echo '<h3>Avoir : ' . $temp->cmd__id_facture . ' le ' . $formate; '</h3><br>';
                                                        }
                                                ?>
...
<?php
                ...
                try {
                        ...
                        if ($temp->cmd__modele_facture != 'AVR') 
                        {
                                $doc->output('C:\laragon\www\SoftRecode\rescue/' . $numFact . 'F-' . $temp->devis__id . 'D-' . $temp->client__id . 'C.pdf', 'F');
                        }
                        else 
                        {
                                $doc->output('C:\laragon\www\SoftRecode\rescue/' . $numFact . 'A-' . $temp->devis__id . 'D-' . $temp->client__id . 'C.pdf', 'F');
                        }
