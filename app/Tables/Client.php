...
class Client extends Table {
  ...
  public function getAll(){
    $request =$this->Db->Pdo->query('SELECT  LPAD(client__id,6,0) as client__id, client__societe ,  client__ville , client__cp  FROM client ORDER BY client__societe DESC LIMIT 150000');
    $data = $request->fetchAll(PDO::FETCH_OBJ);
    return $data;
}

public function getOne($id){
    $request =$this->Db->Pdo->query("SELECT LPAD(client__id,6,0) as client__id,  client__societe , client__adr1 , client__adr2, client__cp , client__ville , client__tel  FROM " .$this->Table. " WHERE client__id = " . $id ."");
